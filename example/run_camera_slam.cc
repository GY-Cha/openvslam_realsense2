...
void mono_tracking(const std::shared_ptr<openvslam::config>& cfg,
	...
	rs2::config rs2_cfg;
	rs2_cfg.enable_stream(RS2_STREAM_FISHEYE,1,RS2_FORMAT_Y8);
	rs2_cfg.enable_stream(RS2_STREAM_FISHEYE,2,RS2_FORMAT_Y8);
	//rs2_cfg.enable_stream(RS2_STREAM_POSE);
   ...
   // }
    ...
    std::thread thread([&]() {
	   ...
	   rs2::frameset frameset = pipe.wait_for_frames();
           rs2::video_frame  fisheye_frame = frameset.get_fisheye_frame(2);
           const int w = fisheye_frame.as<rs2::video_frame>().get_width();
           const int h = fisheye_frame.as<rs2::video_frame>().get_height();
	   // spdlog::critical("width {}", w);
            // check if the termination of SLAM system is requested or not
            cv::Mat gray(cv::Size(w,h), CV_8UC1, (void*)fisheye_frame.get_data(), cv::Mat::AUTO_STEP);
            frame = gray;
...
void stereo_tracking(const std::shared_ptr<openvslam::config>& cfg,
	...
	rs2::frameset frameset = pipe.wait_for_frames();
        ...
        const int w1 = fisheye_frame1.as<rs2::video_frame>().get_width();
        const int h1 = fisheye_frame1.as<rs2::video_frame>().get_height();
	cv::Mat gray1(cv::Size(w1,h1), CV_8UC1, (void*)fisheye_frame1.get_data(), cv::Mat::AUTO_STEP);
            frames[0] = gray1;
        rs2::video_frame  fisheye_frame2 = frameset.get_fisheye_frame(2);
        const int w2 = fisheye_frame2.as<rs2::video_frame>().get_width();
        const int h2 = fisheye_frame2.as<rs2::video_frame>().get_height();
	cv::Mat gray2(cv::Size(w2,h2), CV_8UC1, (void*)fisheye_frame2.get_data(), cv::Mat::AUTO_STEP);
            frames[1] = gray2;
